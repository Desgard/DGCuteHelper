...
#import "DGCuteThemeUI.h"

#import "DGCuteTencentView.h"
#import "DGCuteDotView.h"

@interface DGCuteTencentView()

@property (nonatomic) CGFloat maxDistance;
@property (nonatomic) CGPoint selfCenter;

@property (nonatomic, strong) DGCuteDotView *headDot;
@property (nonatomic, strong) DGCuteDotView *trailDot;
@property (nonatomic, strong) CAShapeLayer *shapLayer;

@end

@implementation DGCuteTencentView


- (instancetype) initWithFrame:(CGRect)frame {
    CGFloat centerX = frame.origin.x + frame.size.width  / 2.f;
    CGFloat centerY = frame.origin.y + frame.size.height / 2.f;
    self.selfCenter = CGPointMake(centerX, centerY);
    ...
    if (self = [super initWithFrame: [UIScreen mainScreen].bounds]) {
        ...
        self.headDot.center = self.selfCenter;
        self.trailDot.center = self.selfCenter;
...
- (void) reloadBeziePath {
    ...
    CGPoint pointN = [self getCenterPointWithX: pointD Y: pointB];
    CGPoint pointM = [self getCenterPointWithX: pointA Y: pointC];
    ...
    [UIView animateWithDuration: 0.25f
         ...
         usingSpringWithDamping: 0.5
          initialSpringVelocity: 0
                     ...
                     animations: ^{
                         self.headDot.center = self.selfCenter;
                         self.headDot.backgroundColor = [UIColor redColor];
                     }
                     completion: ^(BOOL finished) {
...
- (UIView *) hitTest: (CGPoint)point withEvent: (UIEvent *)event {
    ...
    return subView == self.headDot ? subView : nil;
}


#pragma mark - 懒加载
- (DGCuteDotView *) headDot {
    if (!_headDot) {
        _headDot = [[DGCuteDotView alloc] initWithFrame: CGRectMake(0, 0, HEADDOT_W, HEADDOT_W)];
    }
    return _headDot;
}

- (DGCuteDotView *) trailDot {
    if (!_trailDot) {
        _trailDot = [[DGCuteDotView alloc] initWithFrame: CGRectMake(0, 0, TRAILDOT_W_MAX, TRAILDOT_W_MAX)];
    }
    return _trailDot;
}
...
- (CAShapeLayer *) shapLayer {
    if (!_shapLayer) {
        ...
        _shapLayer.fillColor = [UIColor redColor].CGColor;
